...
string SystemManager::get_type_string(const TypeItem type) {
  switch (type) {
  case File:
    return "File";
  case Directory:
...
string SystemManager::concat_lines(const vector<string> &lines,
                                   bool detail_output) {
...
void SystemManager::validate_path(const string &path, TypeItem type) {
  if (!fs::exists(path)) {
    throw_error(
...
bool SystemManager::validate_type_object(const string &path, TypeItem type) {
  switch (type) {
  case File:
...
void SystemManager::list(const string &path, bool recursive, int depth,
                         int current_depth) {
  ...
  for (const auto &entry : it) {
    ...
    cout << format("{:<20} {:<30} {:<20} {:<}", type, time, size, path) << endl;
  }
  cout << "\n";
  if (recursive)
...
void SystemManager::copy_directory(const string &old_path,
                                   const string &new_path, bool recreate,
                                   bool recursive) {
  try {
...
/// @param recreate пересоздать, если существует
void SystemManager::copy_file(const string &old_path, const string &new_path,
                              bool recreate) {
  try {
    ...
    if (fs::exists(new_path)) {
      if (recreate)
        ...
        throw_error(FILE_ALREADY_EXISTS_EXCEPTION,
...
/// @param recreate пересоздать, если существует
void SystemManager::create_directory(const string &path, bool recreate) {
  try {
    if (fs::exists(path)) {
      if (recreate)
        fs::remove_all(path);
...
/// @param path путь, где создать
/// @param recreate пересоздать, если существует
void SystemManager::create_file(const string &path, bool recreate) {
  if (fs::exists(path)) {
    if (recreate)
      fs::remove(path);
    else
...
void SystemManager::modify(const string &old_path, const string &new_path,
                           bool recreate) {
  try {
...
void SystemManager::find(vector<string> &matches, vector<string> &visited,
                         const string &name, const string &path, bool contains,
                         TypeItem type, int limit, bool recursive, int depth,
                         bool log_visited, int current_depth) {
  try {
...
/// @return данные файла
string SystemManager::read(const string &path, bool detail_output) {
...
/// передан)
void SystemManager::insert(const string &path, const string &data,
                           int number_line, int skip) {
  validate_path(path, File);
...
/// @param path расположение директории
void SystemManager::delete_directory(const string &path) {
...
/// @param path расположение файла
void SystemManager::delete_file(const string &path) {
  validate_path(path, File);
  fs::remove_all(path);
};
...
void SystemManager::delete_data_from_file(const string &path, int number_line,
                                          int skip, int length) {
  validate_path(path, File);
