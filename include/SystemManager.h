...
class SystemManager{
    private:
        ...
        string concat_lines(const vector<string>& lines, 
                            bool detail_output = false){
            stringstream concated_data;
            for (size_t i = 0; i < lines.size(); i++){
                if(detail_output){
                    concated_data << i + 1 << ": " << lines[i] << " :" << lines[i].size() << ((i == lines.size() - 1) ? "" : "\n"); 
                }
                else{
    ...
    public:
        ...
        void list(const string& path,
            ...
            cout << format("{:<20} {:<30} {:<20} {:<}", "Type", "Last Write Time", "Size(kb)", "Name") << endl;
        ...
        void copy_directory(const string& old_path, 
        ...
        /// @param recreate пересоздать, если существует
        void copy_file(const string& old_path, 
                       const string& new_path, 
                       bool recreate = false){
            try{
        ...
        void modify(const string& old_path, 
                    const string& new_path, 
                    bool recreate = false){
            try {
        ...
        void find(vector<string>& matches, 
                  vector<string>& visited,
                  const string& what,
                  const string& where = ".",
                  TypeItem type = All,
                  bool recursive_traversal = false,
                  int limit = -1,
                  int max_depth = -1,
                  bool log_visited = false,
                  int current_depth = 0){
            try{
                ...
                for(const auto& entry : it){
                    string path = entry.path().string();
                    if(entry.path().filename().compare(what) == 0){
                    ...
                    if(recursive_traversal && validate_type_object(path, Directory)){
                        if(log_visited){
                            visited.push_back(path);
                        }
                        find(matches, visited, what, path, type, recursive_traversal, limit, max_depth, log_visited, current_depth + 1);
                    }
                }
            }
            catch(const fs::filesystem_error& e){
        ...
        /// @param skip пропуск символов перед вставкой(в конец строки если не передан)
        ...
        void delete_data_from_file(const string& path, 
                                   int number_line = -1, 
                                   int skip = 0, 
                                   int length = -1){
            validate_path(path, File);
